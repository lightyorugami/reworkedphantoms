...
public class PhantomSpawnHandler {
    ...
    public static void onLevelTick(TickEvent.WorldTickEvent event) {
        if (event.world.isClientSide() || !(event.world instanceof ServerWorld)) return;
        ...
        if (
            ...
            ReworkedPhantomsMod.doPhantomSpawning
        ) {
        ...
        if (currentTick > 18000L && currentTick <= 23000L && ReworkedPhantomsMod.doPhantomSpawning) {
            for (ServerPlayerEntity player : level.players()) {
                UUID uuid = player.getUUID();
                ...
                for (UUID id : tracker.spawnedPhantoms) {
                    if (level.getEntity(id) instanceof PhantomEntity) {
                        allDead = false;
                        break;
                    }
                }
    ...
    private static boolean hasAcquireHardware(ServerPlayerEntity player) {
        Advancement adv = player.getServer().getAdvancements()
            .getAdvancement(new ResourceLocation("minecraft:story/smelt_iron"));
        return adv != null && player.getAdvancements().getOrStartProgress(adv).isDone();
    }

    private static List<UUID> trySpawnPhantomGroup(ServerPlayerEntity player, ServerWorld level) {
        List<UUID> phantomIds = new ArrayList<>();
        int spawnCount;
        switch (level.getDifficulty()) {
            case EASY:   spawnCount = 1 + level.getRandom().nextInt(2); break;
            case NORMAL: spawnCount = 2 + level.getRandom().nextInt(2); break;
            case HARD:   spawnCount = 2 + level.getRandom().nextInt(3); break;
            default:     spawnCount = 1; break;
        }
        ...
        for (int i = 0; i < spawnCount; i++) {
            ...
            BlockPos finalPos = level.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, basePos);
            if (finalPos.getY() < basePos.getY()) finalPos = basePos;

            PhantomEntity phantom = EntityType.PHANTOM.create(level);
            if (phantom != null) {
                phantom.getPersistentData().putUUID("OwnerUUID", player.getUUID());
                phantom.moveTo(finalPos, 0.0F, 0.0F);
                phantom.finalizeSpawn(level, level.getCurrentDifficultyAt(finalPos), SpawnReason.NATURAL, null, null);
